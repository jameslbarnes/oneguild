     ...
     mountScrollWorld(document.getElementById('world'), {
       ...
       sections: [
         { id, label, still, stillMobile, clip, clipMobile, accent,
           ...
           more:{label,href},                 // optional link under the body (e.g. a bio)
...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    ...
    c.innerHTML =
      ...
      (s.more && s.more.href ? `<a class="sw-copy__more" href="${esc(s.more.href)}">${esc(s.more.label || 'more')} &#8599;</a>` : '') +
...
function injectCSS() {
  ...
  .sw-copy__more{display:inline-block;margin-top:16px;pointer-events:auto;font-size:.82rem;font-weight:600;color:var(--sw-accent);text-decoration:none;border-bottom:1px solid color-mix(in srgb,var(--sw-accent) 45%,transparent);}
  .sw-copy__more:hover{border-bottom-color:var(--sw-accent);}
